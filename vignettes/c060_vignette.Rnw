...
The elastic net penalty function contains two tuning parameters which are data-dependent and cannot be set equal to some apriori values. The challenge is to find such a setting of tuning parameters $\left(\alpha,\lambda\right)$, for which the k-fold cross validation error of the model is minimal.
...
First, an initial set of uniformly distibuted points in the parameter space is randomly selected. The algorithm learns the Gaussian Process  model from the points  which have been already visited. Then, by adding new points one updates the Gaussian Process model until the opimumum is reached or stopping criteria are met.
...
We can apply the \texttt{glmnet} function to fit a lasso-penalized Cox model to the AML data set. The function call with default penalty parameter settings will fit the lasso model for 100 $\lambda$ data derived values:
...
Controlling a FWER of \Sexpr{fwer} the estimated set of stable features comprises a single gene (with $\hat\Pi >$\Sexpr{pi.thr} at $\lambda =$\Sexpr{round(sel$lambda, 3)}) which is:
...
At the initial step wesample 21 points in the parameter space, Those points are randomly distibuted and cover uniformly the whole interval. Gaussian Process is trained basiedon the initial points. Then,  iteratively next points are added to the Gaussian Process model in order to find an optimal combination of tuning parameters. In total, \Sexpr{nrow(summary_int_search)} were needed to reach the optimum. 
...
\begin{figure}
<<interval_search_cox_out_plot2, echo=F, results=hide, fig=TRUE,height=10>>=

par(las=3)
par(mfrow=c(2,1))
ylim=range(summary_int_search$deviance)
ylim[1]<-ylim[1]-0.01
plot(summary_int_search[, c("alpha", "deviance")] , ylab="Partial Likelihood Deviance", xlab="alpha", type="p", ylim=ylim)
axis(3,at=alphas, tick = TRUE, labels=n.features,cex.axis=0.7 )

rug(summary_int_search[, c("alpha")],side=1, col="black",ticksize = 0.03)
#for (i in 1: (nrow(summary_int_search)-22-1))  rug(summary_int_search[21+i, c("alpha")],side=1, col="blue",ticksize = 0.03+0.005*i)
abline(v=opt.alpha, col="red")

niter<-nrow(summary_int_search) - 21
iter<-c(rep(0,21), c(1:niter))
plot(summary_int_search$alpha, iter, xlab="alpha", ylab="Iteration")
grid(NA, niter+1, lwd=2)
abline(v=opt.alpha, col="red")
@
\caption{\label{fig:interval_search} \textbf{Top}: Deviance as a function of $\alpha$ for the AML data set. The number ofselected features is shown on the top of the plot.The optimal $\alpha$ is highlighted by a red line.
\textbf{Buttom}: The distribution of initial and visited points in the parameter space for the interval search.  }
\end{figure}

Figure \ref{fig:interval_search} illustrates the partial likelihood deviance as a function of the tuning parameter $\alpha$. On the top of the plot corresponding number of features selected are shown. The optimal model is found for small $\alpha$: $\alpha$ =\Sexpr{opt.alpha}. The bottom panel presents the distribution of initial points (iteration=0) and visited points in the parameter space.
This plot shows nicely that the interval search algorithm does not sequentially check up the parameter space, but rather find promising regions and draw a new sample there.   
 The optimal $\alpha$ is highlighted as a red line.
