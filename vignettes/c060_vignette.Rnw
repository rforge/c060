...
The task is to find a setting of tuning parameter values $\left(\alpha,\lambda\right)$, for which the \Sexpr{nfolds}-fold cross-validated penalized (partial) log likelihood function of the model is minimal.  The wrapper function \code{tune.glmnet.interval} calculates partial log likelihood deviance of a model with given tuning parameter setting $\left(\alpha,\lambda\right)$. The parameter space of tuning parameters is defined as follows:
...
<<interval_search_cox_show, echo=TRUE, results=hide,eval=FALSE>>=
  fit <- EPSGO(Q.func="tune.glmnet.interval", 
             bounds=bounds, 
             parms.coding="none", 
             seed = 1234, 
...
%toPrint <- xtable(summary.int[22:31,], caption = "Summary of visited points in the parameter space", label = "tab:sum_int_search2", digits = c(0,5, 7, 5, 0))
%print(toPrint, table.placement = "tbp", caption.placement = "top")
%@
...
#
opt.model<-summary.int$opt.model[[1]]
cofn<-.get.cofn(opt.model)
@
