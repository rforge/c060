...
Although we focus on a survival data application of penalized Cox models in this article, the functions in our \proglang{R} package are in general applicable to all types of regression models implemented in the \pkg{glmnet} package, except for prediction error curves, which are only defined for survival data.
...
allow to use it within the framework of other existing \proglang{R} packages. These functions have been useful in our daily work in our daily work at the
Biostatistics department (C060) of the German Cancer Research Center where prognostic modelling of patient survival data is of particular interest. Therefore, for illustration purposes we focus on penalized Cox PH regression models in this article. But the \proglang{R} functions are applicable to all types of regression models implemented in the \pkg{glmnet} package.
...
binomial models. Since all functions are basically wrapped around the \pkg{glmnet} fitting or cross-validation function call, the type of response is always specified with the \code{family} argument. 

The software is available as \proglang{R} package \pkg{c060} on CRAN (URL \url{http://cran.r-project.org/web/packages/c060/}, version 0.2 at time of manuscript submission).
...
In case no independent validation data are available, resampling-based prediction error curves are used to adequately assess the model's prediction accuracy. The $.632+$ bootstrap estimator \citep{efron1997} is commonly used for these applications, which is a weighted sum of the apparent error and the average out-of-bag bootstrap error. For the apparent error  the same data is used to develop the prognostic model and assess its performance. Due to overfitting, this error is far too optimistic, particularly with high-dimensional data. The average out-of-bag bootstrap error is too conservative since only a proportion of the entire data is used to develop the prognostic model in each bootstrap run. 
...
<<GEOdataGSE12417, echo=FALSE, results=hide>>=
...
<<preprocessData, echo=FALSE, results=hide>>=
...
\begin{figure}[H]
\begin{center}
<<glmnet, echo=TRUE, results=hide, fig=TRUE, height=4>>=
set.seed(1234)
...
<<glmnet3, echo=TRUE, results=hide, fig=FALSE, eval=FALSE>>=
plot.coef.glmnet(cvfit=cvres, betas=names(cofn))
@
\begin{figure}[H]
\begin{center}
<<glmnet4, echo=FALSE, results=hide, fig=TRUE, height=7, width=7>>=
plot.coef.glmnet(cvfit=cvres, betas=names(cofn))
@
\end{center}
...
For this purpose the \proglang{R} package \pkg{peperr} \citep{Porz2009,peperr} provides a modular framework for survival and binary endpoints, i.e., prognostic and
...
Here, we include patient's age as a mandatory model feature into the prognostic model, i.e., age is not subject to penalization, and 
run the calculation on 3 CPUs in parallel using a socket cluster set-up. 

<<peperrMandatoryParallel, echo=TRUE, results=hide, cache=T>>=
...
<<peperrPlot1, echo=TRUE, results=hide, fig=FALSE,eval=FALSE>>=
Plot.peperr.curves(obj, at.risk=TRUE, allErrors=FALSE, bootRuns=FALSE,
                   bootQuants=TRUE, bootQuants.level=0.95, leg.cex=0.7)
@
\begin{figure}[t!]
\begin{center}
<<peperrPlot2, echo=FALSE, results=hide, fig=TRUE,height=5,width=5>>=
Plot.peperr.curves(obj, at.risk=TRUE, allErrors=FALSE, bootRuns=FALSE,
                   bootQuants=TRUE, bootQuants.level=0.95, leg.cex=0.7)
@
\end{center}
...
An alternative implementation of the time-dependent Brier score can be found in the package \pkg{pec} \citep{MIG2012}. The basic approach is similar to \pkg{peperr}, you have to define a wrapper for each fitting procedure in order to determine the estimated survival probabilities. Similar to \pkg{peperr}, no wrapper for \pkg{glmnet} is provided yet. The main difference is that in \pkg{peperr} the tuning of the hyper-parameter and the fitting procedure are done in two separate steps, which more reflects the usual workflow for high-dimensional data analysis. \pkg{pec} on the other hand provides additional functionalities such as the time-dependent c-index in addition to the Brier score but cannot assess classification models. Of course, prediction accuracy of survival models can be assessed differently. Time-dependent ROC and AUC curves are another approach for which different implementations exist provided for example in \proglang{R} packages \pkg{timeROC} \citep{timeROC}, \pkg{survivalROC} \citep{survivalROC} and \pkg{risksetROC} \citep{risksetROC}. The advantage of \pkg{peperr} and \pkg{pec} over most of the other packages is their easy-to-use resampling setup.

For classification models, there are numerous \proglang{R} and Bioconductor packages available to assess models fitted with \pkg{glmnet}. A good starting point is the package \pkg{caret} \citep{caret}.

%\clearpage
\subsection{Stability selection}

So far implementations of stability selection can be found in the packages \pkg{s4vd}, \citep{s4vd,sill2011robust}, \pkg{mboost} \citep{mboost}, \pkg{lol} \citep{lol} and \pkg{BioMark} \citep{BioMark}. While in the package \pkg{s4vd} the stability selection is applied to sparse singular value decomposition in the context of biclustering, the \pkg{mboost} package offers stability selection for model based boosting. The implementation in the \pkg{lol} package is based on penalized, generalized linear models available through the package \pkg{penalized} \citep{penalized}.  As both the \pkg{penalized} and the \pkg{glmnet} package offer penalized models for survival, poisson, binary and gaussian response variables, the stability selection in \pkg{lol} is most comparable to our implemetation. Due to the computational efficiency of the coordinate descent algorithm in \pkg{glmnet} it is probably more appropriate for complex resampling methdos like the stability selection. Morevoer, the code for the stability selection in the \pkg{lol} package is not yet parallelized and does not offer the possibility to compute the whole stability path. The \pkg{BioMark} package offers an implementation of the stability selection for partial least squares (PLS), principal component regression (PCR), variable importance of projection (VIP) and logistic and gaussian \pkg{glmnet} models. Until now the \pkg{BioMark} package allows only to calculate selection frequencies and does not provide type I error control nor the 'randomized lasso' as described in \citet{MeinshausenBuehlmann2010}. \\

Here we use stability selection to identify prognostic features which have a relevant influence on the survival times of the patients in the AML data set and to control Type I errors to ensure that the features identified are truly associated with the survival times.
To calculate the stability path for the $L_1$-penalized Cox regression we use the function \code{stabpath} from our \proglang{R} package. Via the \code{weakness} argument of the function \code{stabpath} it is possible to induce additional randomization by reweighting the penalization of each feature. In brief, in each subsampling step the individual penalization of each feature is randomized such that it lies in the range of $[\lambda,\lambda/\kappa]$, where $\kappa$ is represented by the \code{weakness} parameter which indicates the amount of this additional randomization. The weights $w_{i},\cdots,w_{p}$ to reweight each $\lambda_{i}$ by $\lambda_{i}/w_{i}$ are generated by sampling from a uniform distribution, i.e. $w_{i} \sim \mathcal{U}(\kappa,1)$. \citet{MeinshausenBuehlmann2010} call this additional randomization 'randomized lasso' and showed that it greatly improves the variable selection performance of the stability selection. The function \code{stabpath} draws subsets and calculates in parallel the stability path, e.g., the selection probabilities of each feature along the range of possible penalization parameter values. For parallelization we use the package \pkg{parallel}, which has been a base package since \proglang{R} version 2.14.0. On Unix-like systems the parallelization is done by forking via the function \code{mclapply} whereas under Windows systems socket clusters are used. 

<<stabilitySelection1, echo=TRUE, results=verbatim>>=
...
The task is to find a setting of tuning parameter values $\left(\alpha,\lambda\right)$, for which the \Sexpr{nfolds}-fold cross-validated penalized (partial) log likelihood deviance of the model is minimal.  The wrapper function \\
\code{tune.glmnet.interval} calculates the (partial) log likelihood deviance of a model with given tuning parameter setting $\left(\alpha,\lambda\right)$.

The second tuning parameter $\lambda$ will be found for each given $\alpha$ via the computation of the entire regularization path with the \code{glmnet} function. For each given $\alpha$ an optimal $\lambda$ is defined as the largest value of $\lambda$ such that the loss function value is within one standard error of the minimum (\texttt{type.min ='lambda.1se'}).
...
Summary information can be extracted from the \texttt{fit} object using the \code{summary} function.
<<interval_search_cox_extract_show>>=
sumint <- summary(fit, verbose=TRUE)
@

At the initial step we sample 21 points in the parameter space. Those points are randomly distributed and uniformly cover the whole interval 
(0,1]. A Gaussian process model is trained based on these initial points. Then, iteratively, new points are added to the Gaussian process model in order to find an optimal combination of tuning parameter values. In total, \Sexpr{nrow(sumint$info)} iterations were needed to reach the optimum. 
...
\begin{figure}[t!]
\begin{center}
<<interval_search_cox_out_plot1, results=hide, fig=TRUE,height=8,width=8>>=
plot(sumint)
@
\end{center}
...
This plot shows nicely that the interval search algorithm does not sequentially cover the entire parameter space, but rather quickly finds promising regions and draws new samples there. The optimal model contains with mininmal log-likelihood dieviance is found for $\alpha$ = \Sexpr{round(sumint$opt.alpha,3)} $\log\lambda$ = \Sexpr{round(log(sumint$opt.lambda),3)} and highlighted as a vertical line.

\begin{figure}[t!]
\begin{center}
<<interval_search_cox_out_plot2, results=hide, fig=TRUE,height=8,width=8>>=
...
To our knowledge, the interval search approach have not been applied as an extension of glmnet so far. The only one application of this approach was in our other R package 'penalizedSVM' dealing with feature selection by classification with Support Vector Machines. 
...
\hline
\code{PLL.coxnet} & Predictive partial log-likelihood for glmnet Cox PH model fit\\ 
\code{aggregation.auc} & Determine the area under the ROC curve for a fitted model\\
\code{complexity.glmnet} & Interface for determination of penalty lambda in penalized regression model via cross-validation\\ 
\code{fit.glmnet} & Interface function for fitting a penalized regression model with 'glmnet'\\
\code{plot.coef.glmnet} & Plot the glmnet coefficient path and highlight the path of a pre-specified set of variables\\
\code{plot.peperr.curves} & Plot method for prediction error curves of a peperr object\\
\code{predictProb.coxnet} &  Extract predicted survival probabilities from a coxnet fit \\
\code{predictProb.glmnet} & Extract predicted survival probabilities from a glmnet fit \\
\hline
\code{stabpath} & calculates the stability path for gaussian, binomial, poisson, multinomial and cox \code{glmnet} models\\
\code{stabsel} & estimates a stable set of variables and allows to control the PFER, PCER or FWER\\
\code{plot.stabpath} & displays stability path and indicates estimated stable features\\
\hline
\code{epsgo}              &Finds an optimal solution\\
\code{summary.intsearch}  &Summary method for interval search models\\
\code{tune.glmnet.interval} &Wrapper function for 'glmnet' objects.\\
\code{plot.sum.intsearch} &Plot Summary object for interval search models\\
\hline
\end{tabular}
\caption{\label{tab:functions} Overview of available functions. The first block shows functions to calculate and visualize resampling based prediction errors for penalized Cox and binomial models. The second block shows all provided functions to perform stability selection  and the last block functions to perform interval search.}
\end{table}
\end{center}
%\clearpage
\section{Conclusions and outlook}
The programming language and statistical computing environment \proglang{R} provides a highly useful framework for statistical data analysis and modelling. It is the dominating statistical software in many areas, for example in molecular biology and molecular medicine, which is largely due to the highly successful Bioconductor project \citep{Bioconductor}, which provides tools for the analysis and comprehension of high-throughput genomic data. Due to the open-source nature of \proglang{R} and Bioconductor, many very useful software packages have been developed by \proglang{R} users and made available for the entire \proglang{R} community. One example is the \pkg{glmnet} package, which  implements an efficient state-of-the-art algorithm for fitting penalized Cox and generalized linear models.
