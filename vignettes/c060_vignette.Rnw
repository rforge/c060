...
The task is to find a setting of tuning parameter values $\left(\alpha,\lambda\right)$, for which the \Sexpr{nfolds}-fold cross-validated penalized (partial) log likelihood deviance of the model is minimal.  More precisely, the optimal $\lambda$ is choosen as the largest value of $\lambda$ such that cv-error is within one standard error of the minimum.
...
At the initial step we sample 21 points in the parameter space for $\alpha$ as suggested in the algorithm. Those points are randomly distributed and uniformly cover the whole interval 
...
This plot shows nicely that the interval search algorithm does not sequentially cover the entire parameter space, but rather quickly finds promising regions and draws new samples there. The optimal model contains with minimal log-likelihood deviance is found for $\alpha$ = \Sexpr{round(sumint$opt.alpha,3)} $\log\lambda$ = \Sexpr{round(log(sumint$opt.lambda),3)} and highlighted as a vertical line.
