...
Although we focus on a survival data application of penalized Cox models in this article, the functions in our \proglang{R} package are applicable to all types of regression models implemented in the \pkg{glmnet} package.
...
So far implementations of stability selection can be found in the packages \pkg{s4vd}, \citep{s4vd,sill2011robust},\pkg{mboost} \citep{mboost}, \pkg{lol} \citep{lol} and \pkg{BioMark} \citep{BioMark}. While in the package \pkg{s4vd} the stability selection is applied to sparse singular value decomposition in the context of biclustering, the \pkg{mboost} package offers stability selection for model based boosting. The implementation in the \pkg{lol} package is based on the penalized, generalized linear models available in the package \pkg{penalized} \citep{penalized}.  As both the \pkg{penalized} and \pkg{glmnet} package offer penalized models for survival, poisson, binary and gaussian response variables, the stability selection in \pkg{lol} is comparable to our implemetation.
However, the coordinate descent algorithm in \pkg{glment} might be a computationally more efficient. Morevoer, the code for the stability selection in the \pkg{lol} package is not yet parallelized and does not offer the possibility to compute the whole stability path. The \pkg{BioMark} package offers an implementation of the stability selection for partial least squares (PLS), principal component regression (PCR), variable importance of projection (VIP) and logistic and gaussian \pkg{glmnet} models. Until know the \pkg{BioMark} package allows only to calculate selection frequencies and does not provide type I error control nor the 'randomized lasso' as described in \citet{MeinshausenBuehlmann2010}. \\
...
To calculate the stability path for the $L_1$-penalized Cox regression we use the function \code{stabpath} from our \proglang{R} package. 
...
Here the \code{weakness} parameter allows to induce an additional randomization of the strength of the penalization of each feature. In each subsampling step the individual penalization of each feature is randomized such that it lies in the range of $[\lambda,\lambda/\kappa]$, where $\kappa$ is represented by the the \code{weakness} parameter which measures the amount of this additional randomization. Weights $w_{i},\cdots,w_{p}$ to reweight each $\lambda$ are generated by sampling from a uniform distribution, i.e. $w_{i} \sim \mathcal{U}(\kappa,1)$. \citet{MeinshausenBuehlmann2010} call this additional randomization 'randomized lasso' and showed that it greatly improves the variable selection performance of stability selection.
The function \code{stabsel} can be called to estimate the stable set of features. Controlling a per-family error rate (PFER) of $1$, e.g., expecting one falsely selected feature, the estimated set of stable features comprises a single feature (with $\hat\Pi>0.6$).

<<stabilitySelection2, echo=T, results=verbatim>>=
stabsel(spath, error=1, type="pfer", pi_thr=0.6)$stable
@

Alternativly, \code{stabsel} allows to control the per-comparison wise error rate (PCER) and family-wise error rate (FWER). We provide a  \code{plot} function to visualize the stability path. This function calls \code{stabsel} to estimate stable features and indicates them in the plot (Figure~\ref{stab:path}).
