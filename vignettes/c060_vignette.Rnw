...
The interval search can be divided into two phases. In the initial phase, a set of uniformly distibuted points in the parameter space is randomly selected. Then in the iteration phase, the algorithm learns the Gaussian Process  model from the points  which have been already visited. By adding new points one updates the Gaussian Process model.
New points in the parameter space are sampled by using the expected improvement criterion as described by \citet{Jones1998}, which avoids sticking in local minima. The EPSGO algorithm stops when the global optimum is achieved.

An immense improvement in the training time for the GP compared to that of the classification and regression tasks was be observed \citep{froehlich2005}. Since the number of training points for the GP and hence the number of evaluations of the error surface of the Cox model mainly depends on the dimensionality of the parameter space, which is very small compared to the number of training points for the Cox model. 
...
Controlling a FWER of \Sexpr{fwer} the estimated set of stable features comprises a single feature (with $\hat\Pi >$\Sexpr{pi.thr} at $\lambda =$\Sexpr{round(sel$lambda, 3)}) which is:
...
\clearpage
\subsection{Elastic net penalized Cox PH regression model}

In this section  both $\lambda$ and $\alpha$ are tuned by the interval search. 
...
At the initial step we sample 21 points in the parameter space, Those points are randomly distibuted and cover uniformly the whole interval. Gaussian Process is trained based on the initial points. Then,  iteratively, next points are added to the Gaussian Process model in order to find an optimal combination of tuning parameters. In total, \Sexpr{nrow(summary_int_search)} were needed to reach the optimum. 
...
@
The first five initial 'visited' points in the parameter space ($\alpha$, $\lambda$)  with corresponding cross-validated deviance and number of selected features in each model are presented in figure~\ref{tab:sum_int_search1}.
...
The next section of the summary table shows ten 'visited' points, which are chosen in iteration steps (figure~\ref{tab:sum_int_search2}).
...
#             scales=list(x=list(log=T, equispaced.log = FALSE)), # x axis on log-scale
             ...
             main="cross-validated model deviance",
             sub="number of selected features are printed next to symbol \n rectangles show initial alpha values"))

@
\caption{\label{fig:interval_search1} \textbf{Top}: Deviance as a function of $\alpha$ and log$\lambda$ for the AML data set.  }
\end{figure}

Figure~\ref{fig:interval_search1} illustrates the relationship between both tunig parameters $\alpha$ and $\lambda$ for the 'visited' points. The partial likelihood deviance is  colorcoded with black for small values and gray for large values. 
To distinguish between initial and iteration points, the initial points are plotted as  squares and iteration points as circles. The iteration points were chosen in the regions with lower deviances. The number of features selected is written near each point. 
...
This plot shows nicely that the interval search algorithm does not sequentially check up the parameter space, but rather find promising regions and draw a new sample there.  The optimal model is found for small values: $\alpha$ =\Sexpr{opt.alpha}, which is highlighted as a red line.
...
@
\caption{\label{fig:interval_search2} The distribution of initial and visited points in the parameter space for the interval search plotted in chronological order.  }
\end{figure}
