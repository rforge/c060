...
%\begin{figure}[H]
%\begin{center}
<<glmnet, echo=F, results=hide, fig=FALSE, height=4>>=
set.seed(1234)
...
<<stabilitySelection3, echo=T, results=hide, fig=FALSE,height=7,width=7>>=
plot(spath, fwer=0.5, pi_thr=0.6, xvar="lambda", col.all="gray")
@

\begin{figure}[t!]
\begin{center}
<<stabilitySelection4, echo=F, results=hide, fig=TRUE,height=7,width=7>>=
plot(spath, fwer=0.5, pi_thr=0.6, xvar="lambda", col.all="gray")
@
\end{center}
\caption{Coefficient and stability paths for lasso penalized Cox PH regression model applied to the AML data set. The feature with highlighted path is the only stable feature found by stability selection with FWER=0.5 and $\hat\Pi >0.6$.}
\label{stab:path}
\end{figure}
...
cofn <- coef.opt.intsearch(opt.model)
@
