...
The software is available as \proglang{R} package \pkg{c060} on \proglang{R}-forge (URL \url{http://c060.r-forge.r-project.org}, version 0.2 at time of manuscript submission) and has been submitted to CRAN (URL \url{http://cran.r-project.org}).
