...
Penalized regression models provide a statistically appealing method to build predictive models from high-dimensional data sources. Since the 
...
allows to use it within the framework of other existing \R{} packages. These functions have been useful in our daily work here at the German Cancer Research 
