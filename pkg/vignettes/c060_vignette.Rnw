...
The stability selection proposed by \citet{MeinshausenBuehlmann2010} is a general approach that combines variable selection methodssuch as $L_1$ penalized Cox models with resampling. By applying the corresponding variable selection method to subsamples that were drawn without replacement, selection probabilities for each feature can be estimated. These selection probabilties are used to define a set of stable features. \citet{MeinshausenBuehlmann2010} provide a theoretical framework for controling Type I error rates of falsely assigning features to the estimated set of stable features. The selection probability of each feature along the regularization path, e.g. the range of possible penalization parameters $\Lambda=\{\lambda_1,\lambda_2...,\lambda_k\}$, is called stability path.
...
\end{equation}
where $q_{\Lambda}$ the average of the number of non-zero coefficients w.r.t. to the drawn subsamples.  
Interpreting Equation \ref{stab} the expected number of falsely selected coefficients decreases by either reducing the average number of selected coefficients $q_{\Lambda}$ or by increasing the threshold $\pi_{thr}$. Suppose that $\pi_{thr}$ is fixed, then the stability selection controls $E(V)$ as long as the average number of selected coefficients is less then $e_{\Lambda}$. 
